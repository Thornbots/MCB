[![pipeline status](https://gitlab.com/aruw/controls/MCB/badges/develop/pipeline.svg)](https://gitlab.com/aruw/controls/MCB/-/commits/develop)
...
```
cd my-amazing-project/MCB-project
pipenv shell
# Build for hardware
scons build
# Run automated tests
...
1. Make sure you have VSCode opened in the folder `MCB` (**not
   `MCB-project`**)
...
1. Open the folder `aruw-MCB-project` in VSCode. Hit the debug tab on the left side or type
...
`/MCB-project/robot-type/robot_type.hpp` and change the macro defined in this file.
...
The below commands require that your working directory is `/MCB-project` (where the
