...
namespace ThornBots {
    ...
    */
    void DriveTrainController::setMotorValues(bool sendMotorTimeout, bool useWASD, bool doBeyblading, double right_stick_vert, double right_stick_horz, std::string input) {
        if(useWASD) {
            if(input.length() == 0 && doBeyblading == false) { stopMotors(sendMotorTimeout); } //No inputs. So stop everything?
        ...
        motor_two_speed = getMotorTwoSpeedWithCont(doBeyblading, right_stick_vert, right_stick_horz);
        motor_three_speed = getMotorThreeSpeedWithCont(doBeyblading, right_stick_vert, right_stick_horz);
        motor_four_speed = getMotorFourSpeedWithCont(doBeyblading, right_stick_vert, right_stick_horz);
    ...
    */
    int DriveTrainController::getMotorSetOneTranslatingSpeed(double xPosition, double yPosition) {
        double angle = getAngle(xPosition, yPosition);
    ...
    */
    ...
    int DriveTrainController::getMotorOneSpeedWithWASD(bool doBeyblading) {
        if(doBeyblading) { return 0; }
        //TODO
        return -1;
    }

    int DriveTrainController::getMotorTwoSpeedWithWASD(bool doBeyblading) {
        if(doBeyblading) { return 0; }
        //TODO
        return -1;
    }

    int DriveTrainController::getMotorThreeSpeedWithWASD(bool doBeyblading) {
        if(doBeyblading) { return 0; }
        //TODO
        return -1;
    }

    int DriveTrainController::getMotorFourSpeedWithWASD(bool doBeyblading) {
        if(doBeyblading) { return 0; }
        //TODO
        return -1;
    }
    
    int DriveTrainController::getMotorOneSpeedWithCont(bool doBeyblading, double right_stick_vert, double right_stick_horz) {
        if(doBeyblading) {
            double tmp = -1 * beyblading_factor * max_speed + getMotorSetOneTranslatingSpeed(right_stick_horz, right_stick_vert);
            return ((int) abs(tmp) <= max_speed) ? tmp : ((int) tmp > 0) ? max_speed : -1 * max_speed;
        } else {
            double tmp = right_stick_horz * max_speed + getMotorSetOneTranslatingSpeed(right_stick_horz, right_stick_vert);
            return ((int) abs(tmp) <= max_speed) ? tmp : ((int) tmp > 0) ? max_speed : -1 * max_speed;
        }
    }
    
    int DriveTrainController::getMotorTwoSpeedWithCont(bool doBeyblading, double right_stick_vert, double right_stick_horz) {
        if(doBeyblading) {
            double tmp = -1 * beyblading_factor * max_speed + getMotorSetTwoTranslatingSpeed(right_stick_horz, right_stick_vert);
            return ((int) abs(tmp) <= max_speed) ? tmp : ((int) tmp > 0) ? max_speed : -1 * max_speed;
        } else {
            double tmp = right_stick_horz * max_speed + getMotorSetTwoTranslatingSpeed(right_stick_horz, right_stick_vert);
            return ((int) abs(tmp) <= max_speed) ? tmp : ((int) tmp > 0) ? max_speed : -1* max_speed;
        }
    }
    
    int DriveTrainController::getMotorThreeSpeedWithCont(bool doBeyblading, double right_stick_vert, double right_stick_horz) {
        if(doBeyblading) {
            double tmp = beyblading_factor * max_speed + getMotorSetTwoTranslatingSpeed(right_stick_horz, right_stick_vert);
            return ((int) abs(tmp) <= max_speed) ? tmp : ((int) tmp > 0) ? max_speed : -1 * max_speed;
        } else {
            double tmp = -1 * right_stick_horz * max_speed + getMotorSetTwoTranslatingSpeed(right_stick_horz, right_stick_vert);
            return ((int) abs(tmp) <= max_speed) ? tmp : ((int) tmp > 0) ? max_speed : -1 * max_speed;
        }
    }
    
    int DriveTrainController::getMotorFourSpeedWithCont(bool doBeyblading, double right_stick_vert, double right_stick_horz) {
        if(doBeyblading) {
            double tmp = beyblading_factor * max_speed + getMotorSetOneTranslatingSpeed(right_stick_horz, right_stick_vert);
            return ((int) abs(tmp) <= max_speed) ? tmp : ((int) tmp > 0) ? max_speed : -1 * max_speed;
        } else {
            double tmp = -1 * right_stick_horz * max_speed + getMotorSetOneTranslatingSpeed(right_stick_horz, right_stick_vert);
            return ((int) abs(tmp) <= max_speed) ? tmp : ((int) tmp > 0) ? max_speed : -1 * max_speed;
        }
    }
}
