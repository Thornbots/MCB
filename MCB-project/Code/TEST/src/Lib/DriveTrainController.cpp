...
namespace ThornBots {
    DriveTrainController::DriveTrainController(tap::Drivers* m_driver, std::shared_ptr<CommunicationHandler> ch, std::shared_ptr<HardwareHandler> hh, std::shared_ptr<RefereeSystem> rs) {
        this->m_CommunicationHandler = ch;
    ...
    */
    void DriveTrainController::setMotorValues(bool useWASD, bool doBeyblading, double right_stick_vert, double right_stick_horz, double left_stick_vert, double left_stick_horz, std::string input) {
        if(useWASD) {
            char prev = '\0';
            if(input.length() >= 2) {
